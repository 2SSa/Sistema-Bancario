 ...
 * @author Jose Santos
 */
public class CargarCajero extends JFrame {
    ...
    private void Etiquetas() {
        JLabel etiqueta1 = new JLabel("Area de Carga de Cajero Automatico");
    ...
    private void Botones() {
        ...
        ActionListener oyente2 = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                String expRegular = "[0-9]+;[a-zA-Z0-9 ]+;[0-9]+;(?i)(activo|mantenimiento)";
                String errores = "";
                boolean verificaId;
                if (texto.equals("")) {
                    JOptionPane.showMessageDialog(null, "Ingrese datos", "Error", JOptionPane.ERROR_MESSAGE);
                } else {
                    String[] arrCajero = texto.split("\n");

                    Pattern p = Pattern.compile(expRegular);
                    for (int a = 0; a < arrCajero.length; a++) {
                        Matcher m = p.matcher(arrCajero[a]);
                        if (m.matches() == true) {
                            String[] valores = arrCajero[a].split(";");

                            verificaId = false;
                            //Comprobando si existe el id de la agencia bancaria
                            for (int i = 0; i < Inicio.contadorCajero; i++) {
                                if (Integer.parseInt(valores[0]) == Inicio.cajero[i].getId()) {
                                    verificaId = true;
...
//                                inicio.CrearBanco(valores[0], Integer.parseInt(valores[1]), valores[2], valores[3], Integer.parseInt(valores[4]), Integer.parseInt(valores[5]), Integer.parseInt(valores[6]), true, Integer.parseInt(valores[8]), 0);
                                ...
                                if (valores[3].equalsIgnoreCase("mantenimiento")) {
                                    inicio.CrearCajero(Integer.parseInt(valores[0]), valores[1], Double.parseDouble(valores[2]), false, 0);
                                } else {
                                    inicio.CrearCajero(Integer.parseInt(valores[0]), valores[1], Double.parseDouble(valores[2]), true, 0);
                                }
                            }
